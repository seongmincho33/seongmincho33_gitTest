...
public class RecordingDeviceInfoHandler2 implements RequestHandler<Document, String> {
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        if (document.current.state.reported.mod.equals(document.previous.state.reported.mod) && 
                document.current.state.reported.mod.equals("admin")) {
                return null;
        }
        ...
        return this.dynamoDb.getTable(DYNAMODB_TABLE_NAME)
                .putItem(new PutItemSpec().withItem(new Item().withPrimaryKey("deviceId", document.device)
                        ...
                        .withString("mod", document.current.state.reported.mod)
                        .withString("top", document.current.state.reported.top)
                        .withString("trash_cm", document.current.state.reported.trash_cm)
